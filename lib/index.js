...
function Match(elm, options, data) {
    ...
    this.options.margin = {
        x: options.margin.x || 2
      , y: options.margin.y || 2
    };
...
Match.prototype.render = function (clear) {
    ...
    self.ui.container.innerHTML = self.ui.items.map(function (c) {
        ...
        if (cPos.cX === self.options.size.x) {
            cPos.y += (c.element.offsetHeight + self.options.margin.y);
            cPos.x = 0;
        }

        c.element.style.top = cPos.y;
        c.element.style.left = cPos.x;

        cPos.x += (c.element.offsetWidth + self.options.margin.x);
